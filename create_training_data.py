...
def create_training_case(parent_folder: str, f_name: str, shape: ShapeImage) -> None:
    write_text(join(parent_folder, LABEL_SUB, f_name+LABEL_EXT), shape.label())
...
def generate_test_circles(parent_folder: str, n_tests: int) -> None:
    for idx in range(1, n_tests + 1):
        ...
        create_training_case(parent_folder, 'circle_'+str(idx).zfill(3), shape)
